...
void drive(std::string direction, float target, int velocity, int time){
  ...
  if (direction == "forwards" || direction == "backwards"){
    while (endTime > millis()){
      ...
      if (Catapult_Potentiometer.get_value() > 2000){
...
void reloadCatapult(){
  /*while (Catapult_Potentiometer.get_value() > 2000){
    Catapult.move_velocity(200);
  }
  ...
  Catapult.tare_position();
  Catapult.set_brake_mode(MOTOR_BRAKE_COAST);
  Catapult.move_absolute(2725, 200);*/
}
...
void flagHeight(){
  ...
  while (Left_Arm.get_position() < 302 && Right_Arm.get_position() < 302){
    if (endTime == millis()){break;}
  }
}
